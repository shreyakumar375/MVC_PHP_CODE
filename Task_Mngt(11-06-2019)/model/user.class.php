...
class user
{
    public function getAlltheFeatures($db){
        $fsql = "SELECT TM_feature_id,
        TM_feature_name,TM_feature_tag_name,TM_feature_parent_id
        FROM TM_feature_info AS feainfo
        WHERE TM_feature_isActive=1 and TM_feature_parent_id=0";
        $feature_result = $db->query($fsql)->fetchAll(PDO::FETCH_ASSOC);
        $final_array = array();
        foreach ($feature_result as $key => $value) {
            $fsql1 = "SELECT TM_feature_id,
            TM_feature_name,TM_feature_tag_name,TM_feature_parent_id
            FROM TM_feature_info AS feainfo
            WHERE TM_feature_isActive=1 and TM_feature_parent_id=".$value['TM_feature_id']."";
            $result = $db->query($fsql1)->fetchAll(PDO::FETCH_ASSOC);
            $final_array[$key]=$value;

            $final_array[$key]['sub_feature'] = $result;
        }
    ...
    public function getUserDetails($db,$login_id,$user_name,$password,$c_password,$Email)
    {
        ...
        $final_array = $this->getAlltheFeatures($db);
        ...
        //echo json_encode($featurearray);exit();
        //print_r($featurearray);exit();
        return json_encode(array('MVC'=>array('valid'=>1,'user_result'=>$result,"feature_list"=>$final_array)));
    }
