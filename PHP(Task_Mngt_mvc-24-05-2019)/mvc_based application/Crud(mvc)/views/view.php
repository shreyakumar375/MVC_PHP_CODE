  ...
  <button id="myBtn">CREATEPROFILE</button>
      ...
      <div class="modal-content1">
        ...
        <form action=" " method="post" name="updateForm" id="updateForm">
         <table border=0px" align="center" cellspacing="5px" class="table">
						<tr><td>Name:<td><input type="text" name="edit_name" id="edit_name"></td></tr>
    ...
    window.onclick = function(event) {
     ...
     else if(event.target == modal1)
      {
        modal1.style.display = "none";
			 ...
			 //update function calling using ajax
 			...
 			/*$("body").on("click","#edit", function()
 			 {
 			 	var user_id = $(this).attr('editid');
					       	   ...
					       	   cache:false,
					           type: "POST",
					           url: '?rt=index/fGetUserInfomation',
					           
					           data:{ajax:true,user_id : user_id}
					           }).done(function(data){
					           	if(JSON.parse(data)){
					           var name=$('#edit_name').val(data.MVC.user_result[0].name);
					           		alert(name);
