...
<div class="form-group">
         <label> Employee Confirm_Password:</label><input type="Password" class="form-control" name="empname_cnfrmdpass"id="empname_cnfrmdpass">
       </div>
<div class="form-group">
...
<div class="form-group">
         <label>Select Your_Role:</label> <select>
              ...
              <?php 
              $userRoleList='data.MVC.user_result.length';
                for($i=0;$i<count($userRoleList);$i++){
                  echo '<option value="'.$result[$i]['TM_roleId'].'">'.$result[$i]['TM_roleName'].'</option>';
                }
              ?>
            </select> 
          </div>
    ...
    $.ajax({
      ...
      if(JSON.parse(data)){
        ...
        if(data.MVC.user_deleted == 1){
          alert("User deleted");var empname_cnfrmdpass=$('#empname_cnfrmdpass').val();
...
//edit user details
  ...
  $('#submitbtn').on('click',function finsertfieldVal()
  {
    ...
    if (empname_cnfrmdpass.length==0 && empname_cnfrmdpass=='') 
    {
      alert('enter Employee Confirm Password');
      val=0;
      return false;
    }
    if(empname_pass!=empname_cnfrmdpass)
      {
