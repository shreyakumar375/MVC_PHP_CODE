...
class user
{
    ...
    {
        $sql = "SELECT * from TM_roll_info ";
        $result = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        return json_encode(array('MVC'=>array('valid'=>1,'session'=>1,'role_fetched'=>$result)));
    }
    public function getupdateUserRoleDetails($db,$id)
    {
        $sql="SELECT * from TM_user_info userinfo,TM_roll_info rollinfo WHERE userinfo.TM_info_user_rollid=rollinfo.TM_role_id ";
        $result = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
            //print_r($result);exit();
        return json_encode(array('MVC'=>array('valid'=>1,'session'=>1,'role_fetched'=>$result)));
    }
